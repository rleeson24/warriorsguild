...
{
    public class MustBeSubscriberPolicyProvider : IAuthorizationPolicyProvider
    {
        const string MUST_BE_SUBSCRIBER_POLICY_PREFIX = "MustBeSubscriber";
        public DefaultAuthorizationPolicyProvider FallbackPolicyProvider { get; }

        public MustBeSubscriberPolicyProvider( IOptions<AuthorizationOptions> options )
        {
        ...
        {
            if ( policyName.StartsWith( MUST_BE_SUBSCRIBER_POLICY_PREFIX, StringComparison.OrdinalIgnoreCase ) &&
                int.TryParse( policyName.Substring( MUST_BE_SUBSCRIBER_POLICY_PREFIX.Length ), out var age ) )
            {
                var policy = new AuthorizationPolicyBuilder();
                policy.AddRequirements( new MustBeSubscriberRequirement() );
                return policy.Build();
            }
